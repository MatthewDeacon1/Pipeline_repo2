...
# Class to handle pipeline logic
class Pipeline_Method:
    def __init__(self, progreso_file):
        self.progreso_file = progreso_file
        self.load_progress()

    def load_progress(self):
        # Load progress data from progreso.json
        with open(self.progreso_file, 'r') as file:
            self.progreso_data = json.load(file)
        # Set username to the most recent user
        self.username = self.get_most_recent_user()

    def get_most_recent_user(self):
        # Get the most recent user (last user in the progreso.json file)
        return list(self.progreso_data.keys())[-1]

    def check_and_run_pipeline(self):
        # Iterate through Modulo1 to Modulo5
        for mod_num in range(1, 6):
            modulo_key = f"Modulo{mod_num}"

            # Check if "Leccion2" is true for the current module
            if self.progreso_data[self.username][modulo_key]["Leccion2"]:
                # Construct the file name to check
                file_name = f"Module_{mod_num}_{self.username}.txt"

                # Check if the file exists
                if not os.path.exists(file_name):
                    # If the file doesn't exist, run Pipeline_Dummy.py with the current module number and username
                    subprocess.run(["python", "Pipeline_Dummy.py", str(mod_num), self.username])
                    print(f"Running Pipeline_Dummy.py for {mod_num} and {self.username}")

    def monitor_progress(self):
        # Continuously monitor progreso.json for changes
        while True:
            # Reload progreso.json to check for changes
            self.load_progress()
            self.check_and_run_pipeline()
            # Sleep for a few seconds before checking again (adjust the interval as needed)
            time.sleep(5)


# Initialize and run the pipeline process in a separate thread
def start_pipeline_monitoring(progreso_file):
    pipeline = Pipeline_Method(progreso_file)
    monitor_thread = threading.Thread(target=pipeline.monitor_progress)
    monitor_thread.daemon = True  # Ensures the thread exits when the main program ends
